...
export function BacktestScreen() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Эпизоды бэктеста" description="Проверка исторических стрессовых эпизодов, метрик раннего предупреждения и структуры вкладов модулей." />
      <SectionCard title="Параметры эпизода" description="Выберите стандартный эпизод или задайте собственный диапазон дат.">
        <BacktestControls episode={episode} from={from} onEpisodeChange={setEpisode} onFromChange={setFrom} onToChange={setTo} to={to} />
      </SectionCard>
      {query.isLoading ? <LoadingState label="Загрузка бэктеста..." /> : null}
