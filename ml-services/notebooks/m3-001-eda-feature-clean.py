...
def rolling_mad_score(
    ...
    mad = hist.rolling(window=window, min_periods=min_periods).apply(
        _mad, raw=True
    )
...
def add_base_event_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["cover_ratio"] = safe_divide(
        df["demand_volume_bln_rub"], df["offer_volume_bln_rub"]
    )
    df["cover_ratio_clipped"] = df["cover_ratio"].clip(
        upper=COVER_RATIO_CLIP_UPPER
    )

    df["placement_to_offer_ratio"] = safe_divide(
        df["placement_volume_bln_rub"], df["offer_volume_bln_rub"]
    )
    df["placement_to_demand_ratio"] = safe_divide(
        df["placement_volume_bln_rub"], df["demand_volume_bln_rub"]
    )
    ...
    df["cover_ratio_roll5_mean"] = shifted_cover.rolling(
        window=5, min_periods=1
    ).mean()
    df["cover_ratio_roll5_median"] = shifted_cover.rolling(
        window=5, min_periods=1
    ).median()
    df["cover_ratio_roll5_std"] = shifted_cover.rolling(
...
def build_daily_panel(event_df: pd.DataFrame) -> pd.DataFrame:
    ...
    full_index = pd.date_range(
        START_DATE, df["auction_date"].max(), freq="D", name="date"
    )
    ...
    daily["confirmed_auctions_count"] = (
        daily["confirmed_auctions_count"].fillna(0).astype(int)
    )
    ...
    daily["cover_ratio_clipped"] = daily["cover_ratio"].clip(
        upper=COVER_RATIO_CLIP_UPPER
    )
    ...
    daily["last_auction_date"] = (
        daily["date"].where(daily["has_auction"].eq(1)).ffill()
    )
    daily["days_since_last_auction"] = (
        daily["date"] - daily["last_auction_date"]
    ).dt.days
    daily["days_since_last_auction"] = (
        daily["days_since_last_auction"].fillna(9999).astype(int)
    )
    ...
    daily["auctions_7d_count"] = shifted_has_auction.rolling(
        7, min_periods=1
    ).sum()
    daily["auctions_30d_count"] = shifted_has_auction.rolling(
        30, min_periods=1
    ).sum()
    daily["offer_7d_sum_bln_rub"] = shifted_offer.rolling(
        7, min_periods=1
    ).sum()
    daily["demand_7d_sum_bln_rub"] = shifted_demand.rolling(
        7, min_periods=1
    ).sum()
    daily["placement_7d_sum_bln_rub"] = shifted_placement.rolling(
        7, min_periods=1
    ).sum()
    daily["offer_30d_sum_bln_rub"] = shifted_offer.rolling(
        30, min_periods=1
    ).sum()
    daily["demand_30d_sum_bln_rub"] = shifted_demand.rolling(
        30, min_periods=1
    ).sum()
    daily["placement_30d_sum_bln_rub"] = shifted_placement.rolling(
        30, min_periods=1
    ).sum()
    daily["cover_ratio_30d_mean"] = shifted_cover.rolling(
        30, min_periods=1
    ).mean()
    daily["cover_ratio_90d_mean"] = shifted_cover.rolling(
        90, min_periods=1
    ).mean()
...
def add_stress_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cover_for_signal = (
        ...
        else df["last_cover_ratio"]
    )
    placement_ratio = df.get(
        "placement_to_offer_ratio", pd.Series(np.nan, index=df.index)
    )
    ...
    df["Flag_Low_Cover_MAD"] = (
        (df["MAD_score_cover"] < -2).fillna(False).astype(int)
    )
    df["Flag_Extreme_Oversubscription"] = (
        (cover_for_signal > 5.0).fillna(False).astype(int)
    )
    ...
    if (
        "MAD_score_yield_spread" in df.columns
        and df["MAD_score_yield_spread"].notna().sum() > 30
    ):
    ...
    if "has_auction" in df.columns:
        ...
        df.loc[
            no_auction,
            ["weak_cover_score", "weak_placement_score", "Stress_Score"],
        ] = 0.0
...
def plot_daily_cover_ratio(daily: pd.DataFrame) -> None:
    ...
    plt.plot(
        plot_df["date"],
        plot_df["cover_ratio_clipped"],
        label="Daily auction cover ratio, clipped",
    )
...
def plot_daily_volumes(daily: pd.DataFrame) -> None:
    ...
    plt.plot(
        daily["date"],
        daily["offer_30d_sum_bln_rub"],
        label="Offer, rolling 30d",
    )
    plt.plot(
        daily["date"],
        daily["demand_30d_sum_bln_rub"],
        label="Demand, rolling 30d",
    )
    plt.plot(
        daily["date"],
        daily["placement_30d_sum_bln_rub"],
        label="Placement, rolling 30d",
    )
...
# Keep only OFZ auction data from 2021-01-01 onward.
event_df = event_df.loc[event_df["auction_date"] >= START_DATE].copy()
...
plt.hist(
    daily_df.loc[daily_df["has_auction"].eq(1), "cover_ratio_clipped"].dropna(),
    bins=50,
)
...
plt.plot(
    daily_df["date"],
    daily_df["Stress_Score"],
    label="Daily auction Stress Score",
)
